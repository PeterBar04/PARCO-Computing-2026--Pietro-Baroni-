...
TIME_OUTFILE="time_results.csv"
CACHE_OUTFILE="cache_results.csv"


echo "l1_cache_load,l1_cache_miss,l1_miss_rate,ll_cache_load,ll_cache_miss,ll_miss_rate" > "$CACHE_OUTFILE"

#SEQUENTIAL CODE

g++-9.1.0 -Wall -g -O3 -std=c++17 deliverable1.cpp -o deliverable1
result=$(./deliverable1 none 0) #no schedule and chuck size
time_val=$(echo "$result" | grep "Time" | awk '{printf "%f", $2}')

echo "sequential, $time_val" > "$TIME_OUTFILE"

#penso di dover implementare quella parte di ifndef e fare la funzione seriale

#PARALLEL CODE

g++-9.1.0 -Wall -g -O3 -fopenmp -std=c++17 deliverable1.cpp -o deliverable1

echo "threads,schedule,chunk,run,time" >> "$TIME_OUTFILE"
  ...
  for s in "${SCHEDULES[@]}"; do
    for c in "${CHUNKS[@]}"; do
      ...
      for ((i=1; i<=REPEAT; i++)); do
        result=$(./deliverable1 $s $c)
        time_val=$(echo "$result" | grep "Time" | awk '{printf "%f", $2}')
      done
    
    time_val=${time_val:-0} #if time_val is empty put equal to 0
 
    echo "$t, $s, $c, $i,  $time_val" >> "$TIME_OUTFILE"

#poi devi ordinare i valori in ordine crescente e prendere il nono(90 percentile)
...
for t in "${THREADS[@]}"; do
  ...
  for s in "${SCHEDULES[@]}"; do
    for c in "${CHUNKS[@]}"; do
        ...
        #save on csv
         echo "$t, $s, $c, $avg_l1_load, $avg_l1_miss, $l1_miss_rate, $avg_ll_load, $avg_ll_miss, $ll_miss_rate" >> "$CACHE_OUTFILE"
